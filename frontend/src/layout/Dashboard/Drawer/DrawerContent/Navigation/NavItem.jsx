  ...
  return (
    <>
      <ListItemButton
        ...
        onClick={item.expandable ? handleToggle : () => handlerActiveItem(item.id)}
        selected={isSelected}
        sx={{
          zIndex: 1201,
      ...
      >
        ...
        {item.expandable && (open ? <ExpandLess /> : <ExpandMore />)}
      </ListItemButton>
      {item.expandable && (
        <Collapse in={open} timeout="auto" unmountOnExit>
          <List component="div" disablePadding>
            {item.children.map((childItem) => (
              <NavItem key={childItem.id} item={childItem} level={level + 1} />
            ))}
          </List>
        </Collapse>
      )}
